...
void map_show_sprite(Sprite& sprite, FrameBuffer& fb, Map& map) {
    ...
    fb.draw_rectangle(sprite.x * rect_w - 3, sprite.y * rect_h - 3, 6, 6, pack_color(255, 0, 0)); //3 because we are at the middle of a 6x6 square
}
...
void draw_sprite(Sprite& sprite, std::vector<float>& depth_buffer, FrameBuffer& fb, Player& player, Texture& tex_sprites) {
    ...
    for (size_t i = 0; i < sprite_screen_size; i++) {
        ...
        for (size_t j = 0; j < sprite_screen_size; j++) {
            ...
            uint8_t r, g, b, a;
            unpack_color(color, r, g, b, a); //just to get a from color
            if (a > 128) //a is the transparency number
...
void render(FrameBuffer& fb, Map& map, Player& player, std::vector<Sprite>& sprites, Texture& tex_walls, Texture& tex_monst) {
    ...
    std::vector<float> depth_buffer(fb.w / 2, 1e3); //we save the distance to the wall for each ray
